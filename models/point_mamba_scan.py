...
class PointMambaScan(nn.Module):
    ...
    def forward(self, pts):
        ...
        group_input_tokens_backward = apply_OrderScale(group_input_tokens_backward, self.OrderScale_gamma_2, self.OrderScale_beta_2)
...
class Point_MAE_Mamba_serializationV2(nn.Module):
    ...
    def forward(self, pts, mode='train', vis=False, **kwargs):
        neighborhood, center = self.group_divider(pts)
        ...
        gt_points = neighborhood.reshape(B * G_out, -1, 3)
        
        # 补回各局部 group 的中心坐标，还原到全局的三维绝对坐标系
        center_expanded = center.unsqueeze(2).repeat(1, 1, S, 1).reshape(B * G_out, -1, 3)
