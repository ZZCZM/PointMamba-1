...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        for idx, (taxonomy_ids, model_ids, data) in enumerate(train_dataloader):
            ...
            # 兼容 N_POINTS 的读取位置 (从 others 层级读取)
            if hasattr(config.dataset.train, 'others') and hasattr(config.dataset.train.others, 'N_POINTS'):
                npoints = config.dataset.train.others.N_POINTS
            else:
                npoints = config.dataset.train._base_.N_POINTS
                
            dataset_name = config.dataset.train._base_.NAME
            
            # 将我们的 MiniShiftDataset 加入通过的白名单
            if dataset_name in ['ShapeNet', 'UnlabeledHybrid', 'MiniShiftDataset']:
                points = data.cuda()
